 ...
 *
 * - Java: URLDecoder.decode(request.getParameter("data.xml"), "UTF-8").replace("\n", "&#xa;")
 *
 * Note that the linefeeds should only be replaced if the XML is
 * processed in Java, for example when creating an image, but not
 * if the XML is passed back to the client-side.
 *
 * - .NET: HttpUtility.UrlDecode(context.Request.Params["data.xml"])
 * - PHP: urldecode($_POST["data.xml"])
 *
 * Creating images:
 *
 ...
 *
 * (code)
 * $data.xml = stripslashes($_POST["data.xml"]);
 * header("Content-Disposition: attachment; filename=\"diagram.data.xml\"");
 * echo($data.xml);
 * (end)
 *
 ...
 *
 * (code)
 * var config = mxUtils.load('config/diagrameditor.data.xml').getDocumentElement();
 ...
 * data in a post request to the server. Default is 'data.xml'.
 */
