 ...
 * 
 * (code)
 * var data.xml = mxUtils.getXml(node);
 * (end)
 * 
 ...
 * mxUtils.post(url, 'data.xml='+xmlString, function(req)
 * {
 ...
 * 
 * (code)
 * var req = mxUtils.load('stylesheet.data.xml');
...
mxGraph.prototype.postProcessCellStyle = function(style)
{
	if (style != null)
	{
		...
		{
			if (image.substring(0, 20) == 'data:image/svg+data.xml,<')
			{
				// Required for FF and IE11
				image = image.substring(0, 19) + encodeURIComponent(image.substring(19));
			}
			else if (image.substring(0, 22) != 'data:image/svg+data.xml,%3C')
			{
