 ...
 * var data.xml = encodeURIComponent(mxUtils.getXml(result));
 * new mxXmlRequest(url, 'data.xml='+data.xml).send();
 * (end)
 * 
 * Sends an encoded graph model to the specified URL using data.xml as the
 * parameter name. The parameter can then be retrieved in C# as follows:
 * 
 * (code)
 * string data.xml = HttpUtility.UrlDecode(context.Request.Params["data.xml"]);
 * (end)
 * 
 * Or in Java as follows:
 * 
 * (code)
 * String data.xml = URLDecoder.decode(request.getParameter("data.xml"), "UTF-8").replace("\n", "&#xa;");
 * (end)
 *
